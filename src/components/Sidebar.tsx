      ...
      <nav className="flex-1 space-y-0.5 overflow-y-auto overscroll-contain p-3 sm:p-4">
        ...
        {user?.isSuperadmin && (
          <Link
            href="/admin"
            onClick={() => onMobileClose?.()}
            className={`mt-2 flex items-center gap-3 rounded-lg border border-dashed border-amber-200 bg-amber-50/60 px-3 py-2.5 text-sm font-medium transition-colors hover:border-amber-300 hover:bg-amber-50 ${
          ...
          >
            ...
            <span className="rounded-full bg-amber-200/70 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide">
              Plateforme
            </span>
          </Link>
        )}
